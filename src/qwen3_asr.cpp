...
// ---------- P2-A: conv frontend ----------
static vector<float> conv2d(const vector<float>&in,int Cin,int Hin,int Wi,const vector<float>&W,const vector<float>&b,int Cout,int&Ho,int&Wo){
    Ho=convlen(Hin);Wo=convlen(Wi);vector<float> out((size_t)Cout*Ho*Wo);
    #pragma omp parallel for
    for(int oc=0;oc<Cout;oc++)for(int oh=0;oh<Ho;oh++)for(int ow=0;ow<Wo;ow++){double s=b[oc];
        for(int ic=0;ic<Cin;ic++){const float*ip=&in[(size_t)ic*Hin*Wi];const float*w=&W[(((size_t)oc*Cin+ic)*3)*3];
            for(int kh=0;kh<3;kh++){int ih=oh*2-1+kh;if(ih<0||ih>=Hin)continue;for(int kw=0;kw<3;kw++){int iw=ow*2-1+kw;if(iw<0||iw>=Wi)continue;s+=ip[ih*Wi+iw]*w[kh*3+kw];}}}
        out[((size_t)oc*Ho+oh)*Wo+ow]=gelu((float)s);}
    return out;}
static vector<float> frontend(const vector<float>&mel,int T,int&N){
    const int NMEL=128,WIN=100,DSH=480,D=896,FDIM=16;
    auto c1w=E("conv2d1_weight",(size_t)DSH*9),c1b=E("conv2d1_bias",DSH);
    auto c2w=E("conv2d2_weight",(size_t)DSH*DSH*9),c2b=E("conv2d2_bias",DSH);
    auto c3w=E("conv2d3_weight",(size_t)DSH*DSH*9),c3b=E("conv2d3_bias",DSH);
    auto cow=E("conv_out_weight",(size_t)D*DSH*FDIM),pe=E("positional_embedding",(size_t)1500*D);
    vector<int> cl;int rem=T;while(rem>0){int c=rem>=WIN?WIN:rem;cl.push_back(c);rem-=c;}
    vector<float> ein;
    for(size_t ci=0;ci<cl.size();ci++){vector<float> ch((size_t)NMEL*WIN,0.f);int base=(int)ci*WIN;
        for(int m=0;m<NMEL;m++)for(int tt=0;tt<WIN;tt++){int t=base+tt;if(t<T)ch[(size_t)m*WIN+tt]=mel[(size_t)m*T+t];}
        int H1,W1,H2,W2,H3,W3;auto o1=conv2d(ch,1,NMEL,WIN,c1w,c1b,DSH,H1,W1);auto o2=conv2d(o1,DSH,H1,W1,c2w,c2b,DSH,H2,W2);auto o3=conv2d(o2,DSH,H2,W2,c3w,c3b,DSH,H3,W3);
        int valid=convlen(convlen(convlen(cl[ci])));
        for(int t=0;t<valid;t++){vector<float> feat((size_t)DSH*H3);for(int c=0;c<DSH;c++)for(int f=0;f<H3;f++)feat[c*H3+f]=o3[((size_t)c*H3+f)*W3+t];
            for(int o=0;o<D;o++){const float*w=&cow[(size_t)o*DSH*H3];double s=0;for(int k=0;k<DSH*H3;k++)s+=feat[k]*w[k];ein.push_back((float)s+pe[(size_t)t*D+o]);}}}
